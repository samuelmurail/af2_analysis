...
from af2_analysis import new_clust
...
def test_cf_1_5_5_relax_new_clust(mock_show):
    ...
    clust_num = len(my_data.df['cluster'].unique())
    assert clust_num == 6
    assert list(my_data.df['cluster']) == [3, 2, 1, 1, 1, 4, 2, 1, 5, 6, 3, 2, 1, 1, 1, 2, 2, 1, 1, 1, 3, 2, 1, 1, 1, 3, 2, 1, 5, 6, 4, 2, 1, 1, 1, 3, 2, 1, 1, 1]
    ...
    assert clust_num == 18
    print([i for i in my_data.df['cluster']])
    assert list(my_data.df['cluster']) == [6, 7, 1, 9, 2, 10, 4, 11, 12, 8, 6, 13, 1, 3, 2, 14, 15, 1, 3, 2, 5, 4, 1, 3, 2, 5, 7, 1, 16, 8, 17, 18, 1, 3, 2, 5, 4, 1, 3, 2]
