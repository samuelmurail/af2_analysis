...
Clustering protein structure models generated by AlphaFold. This approach aims to address the challenge of managing and analyzing the large number of models (e.g., 10.000) produced for each protein complex, especially since these models often exhibit structural redundancies.


Il aligne tous
les récepteurs de chaque ensemble de modèles fournis avant de calculer les matrices de RMS (Root
Mean Square Deviation) entre les résidus de l’interface des différents peptides. En raison de valeurs
parfois trop élevées, nous avons passé les matrices
normalisation ,𝑹𝑴𝑺 𝒔𝒄𝒂𝒍𝒆𝒅 ( 𝑹𝑴𝑺 , 𝒅𝒊) =
𝟏
𝟏
𝑹𝑴𝑺
²
𝒅𝒊
par une fonction de mise à l'échelle ou
, décrite par Björn Wallner (8) avec: RMS:
matrice des RMSD, di : facteur de mise à l’échelle de 8.5 Å . Des matrices de distance seront obtenues
où les valeurs proches de 0 correspondent à des structures similaires et celles proches de 1 à des
structures différentes. À partir de cette matrice, nous avons effectué une classification hiérarchique
ascendante pour déterminer les clusters.
